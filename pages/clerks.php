...
$sql = "SELECT * FROM `clerks` WHERE `status`='active'";
$result = $conn->query(($sql));

$sql_trips = "SELECT * FROM trips WHERE `status`='active' AND departure_date > CURDATE()";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_clerk'])) {
        ...
        if (empty($errors)) {
            $stmt = $conn->prepare("INSERT INTO `clerks` (`username`, `email`, `password`, `image_url`, `status`) VALUES (?,?,?,?,?)");
            if (!$stmt) {
    ...
    if (isset($_POST['delete_clerk'])) {
        ...
        $status = "inactive";
        $stmt = $conn->prepare("UPDATE `clerks` SET `status`=? WHERE `id`=?");
        if (!$stmt) {
            die("Prepare failed:" . $conn->error);
        }
        $stmt->bind_param("si", $status, $clerk_id);
        if ($stmt->execute()) {
    ...
    <table class="main-operations">
        ...
        <tbody>
            ...
            if ($result->num_rows > 0) {
                ...
                echo "<tr><td colspan='8'>No Clerks found</td></tr>";
            }
            ?>
...
<div class="popup-form">
    <div class="popup-content">
        <div class="auth-form animate__animated animate__zoomIn">
            ...
            <form method="post">
                <input type="hidden" name="add_clerk" />
    ...
    <div class="popup-content">
        <div class="delete-modal animate__animated animate__zoomIn">
            ...
            <div class="forms-modal delete-section-button">
                <form method='post'>
                    <input type="hidden" name="delete_clerk" value="1" />
                    <input type="hidden" name="clerk_id" id="delete-id" />
